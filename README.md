...
Project retrieval uses deterministic term frequency by default. When two current files carry different authority (for example, a sealed release handoff superseding an older operating note), set an explicit bounded project-scoped level with `knowledge authority SOURCE_ID --project PROJECT --level -100..100`. Explicitly named filenames still rank first; otherwise authority ranks before term frequency. Level `0` removes the override. Search previews and frozen evidence expose the applied level, and this command never calls a model or changes a source file.
