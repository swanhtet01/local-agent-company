...
6. Open the mission ID in the dashboard; compare its report and evidence-manifest SHA-256 values; inspect every frozen excerpt, failed gate, and source conflict; then read the Markdown report and verify its assumptions yourself. Verified wording must cite the displayed `[EVIDENCE:id]`, but the citation remains evidence for owner review rather than proof of a business outcome. After quality-rule changes, use **Current preview** (or `quality JOB_ID --preview`) first: it evaluates a disposable clone and changes no real state. Use **Recheck** only after reviewing that comparison because a recheck appends to evaluation history and can change the linked queue status. Use **Retry** only after the failed constraint or model problem is corrected. Review **Retired failure proofs** or run `queue supersession-list`; investigate every `review_required` record without rewriting its historical audit result. If an exact newer retry lineage has made an active failed queue item obsolete, first run `queue supersession-preview QUEUE_ID` or open **Supersession proof**. Retire it only with the returned exact successor: `queue supersede QUEUE_ID --successor-job RETRY_JOB_ID --reason "..."`. These checks call no model; the mutation revalidates the descendant lineage, current evaluation, report seal, and evidence manifest before removing only the active recovery signal. It preserves the queue, jobs, reports, evaluations, and proof-bearing audit event and is reversible with `queue reset`. A reason without successor proof fails closed.
